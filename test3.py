...
def main():
    ...
    print("Step 3: Str are added to the mz file!")

    npdb = NPDB_to_pd("DB.sqlite")
    print("Step 4: DB is converted to a dataframe!")
    results_df = []
    ...
    results_df = pd.concat(results_df).reset_index(drop=True)
    print("Step 5: DB is queried with the curated table 2 dataframe!")

    # write first set of files
    if not results_df.empty:
        print("Step 6: Writing 1st set of files!")
        res_file = os.path.join("./MassDB", 'full.results.csv')
        ...
        res2_file = os.path.join("./MassDB", 'NPDB_entries.txt')
